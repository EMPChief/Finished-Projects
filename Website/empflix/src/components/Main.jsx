...
const styles = {
  ...
  backgroundImageOverlay: 'absolute w-full h-full bg-gradient-to-r from-black opacity-60',
  movieImage: 'w-full h-full object-cover',
  containEverything: 'absolute inset-0 flex flex-col justify-center items-left p-4 md:p-8',
  ...
  return (
    <div className={styles.mainContainer}>
      <div className={styles.contentContainer}>
        ...
        <img className={styles.movieImage} src={`https://tmdb.org/t/p/original/${movie?.backdrop_path}`} alt={movie?.title} />
        <div className={styles.containEverything}>
          ...
          <p className={styles.overview}>{truncateString(movie?.overview, 150)}</p>
        </div>
      </div>
    </div>
  );
